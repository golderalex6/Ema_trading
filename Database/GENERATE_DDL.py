...
on Ema_{i}
begin
    delete from Ema_{i} where Timestamp in (select Timestamp from Ema_{i} order by Timestamp limit 1) and (select count(*) from Ema_{i})>=120;
end;\n
'''
...
on Price_{i}
begin
    delete from Price_{i} where Timestamp in (select Timestamp from Price_{i} order by Timestamp limit 1) and (select count(*) from Price_{i})>=120;
end;\n
'''
